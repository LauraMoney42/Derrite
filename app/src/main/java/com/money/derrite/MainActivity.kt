...
// Translation imports
import com.money.derrite.translation.AdaptiveTranslator
import com.money.derrite.translation.SimpleTranslator
...
class MainActivity : AppCompatActivity() {
    ...
    private fun startLocationUpdates() {
        try {
            ...
            val locationCallback = object : com.google.android.gms.location.LocationCallback() {
                override fun onLocationResult(locationResult: com.google.android.gms.location.LocationResult) {
                    try {
                        ...
                        android.util.Log.e("Derrite", "Error in location callback: ${e.message}")
                    }
                }
            }
            ...
            android.util.Log.e("Derrite", "Permission denied for location updates")
        } catch (e: Exception) {
            android.util.Log.e("Derrite", "Error starting location updates: ${e.message}")
        }
    }
                    ...
                    if (::mapView.isInitialized && mapView.repository != null) {
                        ...
                        Handler(Looper.getMainLooper()).postDelayed({
                            try {
                                ...
                                android.util.Log.e("Derrite", "Error in delayed report loading retry: ${e.message}")
                            }
                        }, 2000)
                    }
                } catch (e: Exception) {
                    android.util.Log.e("Derrite", "Error in delayed report loading: ${e.message}")
                }
            }, 2000)
        } catch (e: Exception) {
            activeReports.clear()
            android.util.Log.e("Derrite", "Error loading saved reports: ${e.message}")
        }
    }
    ...
    private fun showPreferencesDialog() {
        ...
        addPreferenceSection(dialogLayout,
            ...
            if (isSpanish) "Requerido para alertas" else "Required for alerts"
        )
    ...
    private fun showPrivacyPolicyDialog() {
        ...
        val content = if (isSpanish) {
            ...
            Tu privacidad es nuestra principal prioridad.
            """.trimIndent()
        } else {
            """
            Privacy Policy
    ...
    private fun showAboutDialog() {
        ...
        val content = if (isSpanish) {
            ...
            Creado para las comunidades, por las comunidades. Tu privacidad es nuestra prioridad.
            
            Servidor: Railway Cloud (Global)
            Versión: ${try { packageManager.getPackageInfo(packageName, 0).versionName } catch (e: Exception) { "1.0.0" }}
            
            Hecho con cuidado para comunidades más informadas.
            """.trimIndent()
        } else {
            """
            About Derrite
            ...
            COMMUNITY DRIVEN
            Created for communities, by communities. Your privacy is our priority.
            
            Backend: Railway Cloud (Global)
            Version: ${try { packageManager.getPackageInfo(packageName, 0).versionName } catch (e: Exception) { "1.0.0" }}
            
            Made with care for more informed communities.
            """.trimIndent()
        }

        showInfoDialog(if (isSpanish) "Acerca de Derrite" else "About Derrite", content)
    }
    ...
    private fun addSearchResultMarker(location: GeoPoint, address: Address) {
        try {
            ...
            android.util.Log.e("Derrite", "Error adding search marker: ${e.message}")
            Handler(Looper.getMainLooper()).postDelayed({
                try {
                    addSearchResultMarker(location, address)
    ...
    private fun setupMap() {
        try {
            ...
            android.util.Log.e("Derrite", "Error setting up map: ${e.message}")
            try {
                mapView.setTileSource(TileSourceFactory.MAPNIK)
            } catch (fallbackE: Exception) {
                android.util.Log.e("Derrite", "Fallback map setup also failed: ${fallbackE.message}")
            }
        }
    }
        ...
        try {
            ...
            markerToRemove?.let { marker ->
                try {
                    ...
                    android.util.Log.e("Derrite", "Error removing marker: ${e.message}")
                }
            }
            ...
            if (reportCircles.isNotEmpty()) {
                try {
                    ...
                    android.util.Log.e("Derrite", "Error removing circle: ${e.message}")
                }
            }

            mapView.invalidate()
        } catch (e: Exception) {
            android.util.Log.e("Derrite", "Error in removeReportFromMap: ${e.message}")
        }
    }
    ...
    private fun addLocationMarker(location: GeoPoint) {
        try {
            ...
            android.util.Log.e("Derrite", "Error adding location marker: ${e.message}")
            Handler(Looper.getMainLooper()).postDelayed({
                try {
                    addLocationMarker(location)
                } catch (retryE: Exception) {
